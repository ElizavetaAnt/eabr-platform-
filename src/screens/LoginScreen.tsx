...
export function LoginScreen() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    console.log('Profile fetch result:', { profile, profileError, userId: data.user.id })

    if (profileError) {
      setError(`Ошибка профиля: ${profileError.message} (${profileError.code})`)
      setLoading(false)
      return
    }

    if (!profile) {
      setError(`Профиль не найден для ID: ${data.user.id}`)
      setLoading(false)
      return
    }
